                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <tr v-for="order in orderList" :key="order.id">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                <router-link :to="`/admin/order/details/${order.order_code}`" class="hover:text-green-400 hover:underline">{{ order.order_code }}</router-link>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ order.total_amt }} MMK
                            </td>
...
const getOrder = async () => {
    try {
        const res = await axiosInstance.get(`/api/order/get/orders`);
        if (res.data) {
